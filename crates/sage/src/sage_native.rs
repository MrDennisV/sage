...
use crate::{Error, FsStore, Result, Sage, peers::Peers};
...
impl Sage {
    pub fn new(path: &Path, test: bool) -> Self {
        Self {
            ...
            store: Box::new(FsStore::new(path.to_path_buf())),
...
impl<E: SqlExecutor> Sage<E> {
    pub fn wallet_db_path(&self, fingerprint: u32) -> Result<PathBuf> {
        ...
        let path = path.join(format!("{}.sqlite", self.network_id_of(fingerprint)));
        Ok(path)
    }
